...
const App: React.FC = () => {
  ...
  const [shoppingItems, setShoppingItems] = useState<ShoppingItem[]>(() => {
    ...
    return saved ? JSON.parse(saved) : SHOPPING_LIST_ITEMS;
  });
  ...
  // Persistence Effects with Debounce
  useDebouncedEffect(() => { localStorage.setItem('tasks', JSON.stringify(tasks)); }, [tasks], 1000);
  useDebouncedEffect(() => { localStorage.setItem('notices', JSON.stringify(notices)); }, [notices], 1000);
  useDebouncedEffect(() => { localStorage.setItem('shoppingItems', JSON.stringify(shoppingItems)); }, [shoppingItems], 1000);
  useDebouncedEffect(() => { localStorage.setItem('files', JSON.stringify(files)); }, [files], 1000);
  ...
  return (
    <div className={`min-h-screen flex flex-col relative overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-green-900 via-slate-900 to-purple-900 text-slate-100 selection:bg-purple-900' : 'bg-slate-50 text-slate-900 selection:bg-gold-200'}`}>
      ...
      <header className={`sticky top-0 z-40 backdrop-blur-xl border-b px-8 py-5 flex justify-between items-center shadow-sm transition-colors ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white/90 border-slate-100'}`}>
      ...
      <div className="bg-gradient-to-r from-gold-400 via-gold-500 to-gold-600 text-black py-2 text-center text-[11px] font-black uppercase tracking-[0.2em] shadow-lg relative z-30 border-b border-gold-300">
          ...
          <div className="max-w-7xl mx-auto space-y-10 pb-40">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              <div className="lg:col-span-8 space-y-10">
                ...
                <div className={`border-2 p-10 rounded-[3rem] relative overflow-hidden shadow-2xl transition-colors ${isDarkMode ? 'bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 border-slate-700' : 'bg-gradient-to-br from-gold-50 via-white to-gold-50 border-gold-200'}`}>
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
                    <div className="space-y-2">
                      <h2 className={`text-3xl font-black flex items-center gap-3 tracking-tighter ${isDarkMode ? 'text-gold-400' : 'text-gold-950'}`}>
                        <span className="text-4xl">✨</span> Cajun Growth Strategist
                      </h2>
                      <p className={`text-base font-bold leading-tight ${isDarkMode ? 'text-slate-400' : 'text-gold-800/80'}`}>Advanced AI analysis for thecajunmenu.com. Strategies include National Average benchmarks.</p>
                    </div>
                    <button
                      onClick={fetchAiSuggestions}
                      disabled={aiLoading}
                      className="bg-gradient-to-b from-gold-300 via-gold-500 to-gold-700 hover:from-gold-200 hover:via-gold-400 hover:to-gold-600 text-white px-10 py-5 rounded-2xl font-black shadow-2xl shadow-gold-500/40 border-t border-gold-300 transition-all disabled:opacity-50 active:scale-95 text-lg shine-btn"
                    >
